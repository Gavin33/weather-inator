...
const Weather = (props) => {
  ...
  const getTemp = async () => {
    ...
    const authenticate = (input) => {
      if (!input) {
        setWeather(<div>Please enter a zip code</div>);
      } else if (isNaN(input)) {
      ...
      for (let i = 0; i < invalidZip.length; i++) {
        ...
        setWeather(<div>New number wat dis?</div>);
      } else if (input.length !== 5) {
        setWeather(<div>Only 5 digit zip codes are supported</div>);
      } else if (!validateZip(input)) {
        setWeather(<div>Invalid zip</div>);
      } else {
        return true;
      }
      return false;
